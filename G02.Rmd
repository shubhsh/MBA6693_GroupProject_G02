...
merged_data %>%
  ...
  theme_minimal()+
  scale_fill_manual(values = cp_2) +
  labs(x="Outstanding Balance", fill="Performance Tag", title="Outstanding Balance vs Performance Tag") 
...
```{r}
prComp<-prcomp(data_explore[,-1],scale. = TRUE)
std_dev <- prComp$sdev
...
```{r}
train.data<-data.frame(Performance_Tag=as.factor(data_explore$Performance_Tag), prComp$x)
...
```{r}
model_rf <- train(Performance_Tag~.,data=train.data, method="rf", tuneGrid=tunegrid, trControl=control)
```
...
```{r}
write.csv(woe_data_woe,file="woe_data_woe.csv")
```
...
set.seed(123) 
split = sample.split(merged_data$Performance_Tag, SplitRatio = 0.70) 
...
```{r}
model <- glm(formula = Performance_Tag ~ Avg_CC_Utilization_12_months_woe+Trades_12_months_woe+PL_Trades_12_months_woe+Inquiries_12_months_woe+Outstanding_Balance_woe+No_Of_30_DPD_6_months_woe+Total_No_of_trades_woe+PL_Trades_6_months_woe+No_Of_90_DPD_12_months_woe+No_Of_60_DPD_6_months_woe+Inquiries_6_months_woe, data=woe_data_woe, family=binomial)
```
...
```{r}
y_pred = predict(model, newdata = test_set[-1]) 
